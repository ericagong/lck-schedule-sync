...
**Phase 1 완료 → Phase 2 진입 직전**. LCK 모든 단계(정규/플옵/결승/플레이-인) stage SUMMARY 표시 검증 끝. TBD 매치는 ICS 미포함 정책 유지(매일 2회 cron이 lag 흡수). 2025 historical fixture로 캘린더 시각 검증까지 완료. 단위 테스트 38개.
...
Phase 3:        + 네이버 esports JSON API 자동화 (EWC + KeSPA Cup + 아시안 게임) — T1 출전 확정 시 반응형
...
| 후보                            | 탈락 사유                                 |
| ------------------------------- | ----------------------------------------- |
| lolesports API 확장             | 41개 리그 전수 확인, EWC·KeSPA·AG 미커버  |
