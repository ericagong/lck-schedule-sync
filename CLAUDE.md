...
**Phase 1 완료 → Phase 2 진입 직전**. LCK 모든 단계(정규/플옵/결승/플레이-인) stage SUMMARY 표시 검증 끝. TBD 매치는 ICS 미포함 정책 유지(매일 2회 cron이 lag 흡수). 2025 historical fixture로 캘린더 시각 검증까지 완료. 단위 테스트 33개.
...
Phase 2 (다음): + MSI + Worlds + First Stand (lolesports API, T1 출전분만 t1.ics 통합)
Phase 3:        + 수동 등록 데이터 (EWC + KeSPA Cup + 아시안 게임) — 새 데이터 어댑터, 반응형 (T1 출전 확정 시)
...
- **EWC·KeSPA Cup·아시안 게임**: lolesports API 미커버 → Phase 3에서 수동 입력 (T1 출전 확정 시 반응형)
